...
AUTORES: SANCHEZ TIRADO CITLALLI YASMIN
		JIMENEZ DELGADO LUIS DIEGO
		AARON GAMALIEL 
VERSION: 1.0		
...
void pintar(int filaPintar, int Posicion, elemento Cliente, char acaboFila){
	...
	if(acaboFila == 'S'){ //Verificamos si ya no hay espacio en la fila
		for(i=0; i<x; i++){ //Hace un ciclo para limpira la fila
...
int main(void){
	...
	while(fin == 'N'){ //Hacemos un ciclo que representara el tiempo en que esta funcionando el supermercado
		...
		for(i=0; i<cantCajeras; i++){ //Comenzamos un ciclo desde 0 hasta el numero de cajeras	
		...
		for(i=0; i<cantCajeras; i++){ //Inciamos un ciclo que vaya desde 0 hasta el numero de cajeras
			for(k=0; k<=Size(&filaCajera[i]); k++){ //Ciclo que vaya desde cero hasta el tamano de la fila
				// printf(" %d",k);
		...
		for(a=0; a<cantCajeras; a++){ //Verificamos que todas las cajas esten funcionando
